<template>
  <v-card width="300" :loading="loading">
    ...
    <div>
      <v-card flat color="lighten-2" height="150">
        <v-img v-if="showAsset" :src="value" eager contain width="100%" height="100%"/>
        <div v-else class="full-width fill-height no-selection align-center d-flex align-center justify-center">
          <div class="grey--text">
            No selection
          </div>
        </div>
    ...
    <div>
      ...
      <v-row no-gutters v-else-if="hasAssets">
        <v-col no-gutters cols="9">
          <v-btn width="100%" color="primary" depressed dark tile @click="$refs.inputUpload.click()">
            <v-icon class="mr-2">
              mdi-cloud-upload
            </v-icon>
            {{buttonLabel}}
        ...
        <v-col no-gutters cols="3">
          <v-btn width="100%" color="primary" outlined dark tile @click="displayDialog">
            <v-icon>
              mdi-folder-open
            </v-icon>
          </v-btn>
      ...
      <v-row no-gutters v-else-if="!hasAssets">
        <v-col no-gutters cols="12">
          <v-btn width="100%" color="primary" dark tile @click="$refs.inputUpload.click()">
            <v-icon class="mr-2">
              mdi-cloud-upload
            </v-icon>
            {{buttonLabel}}
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            removeAsset() {
                let newValue = this.default;
                this.$emit('input', newValue);
            },
            selectAsset(asset) {
                let newValue = asset.link;
                this.$emit('input', newValue);
